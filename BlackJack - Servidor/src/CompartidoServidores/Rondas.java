...
//                }
                ...
                /*Aqui tenemos al menos a un jugador no banca*/
            ...
            else if (this.getQuierenJugar()==0 && fase==0 && this.ultimoJugador()>=0){
                System.out.println("aqui porque entramos en el cambiar fase a espera??");
            this.turno=-1;
            this.setRondaEspera(false);
            this.siguienteTurno();
            }
    ...
    public int ultimoJugador(){
        ...
        while (i>=0 && !encontrado){
             if (this.jugadoresRonda[i].isJugando())
             {encontrado=true;
             pos = i;
             }
            i--;
    ...
    public int eliminarJugador(Jugador j){
        ...
        if (i<3) {
            ...
            System.out.println("esta si que es definitiva... !!!!");
            if (!this.jugadoresRonda[i].isEsperando()) this.quierenJugar--;
            if (turno/2==i){

                System.out.println("es el turno del jugador estepor tanto hemos de hacer sigturno");
    ...
    public boolean addApuestaTurnos(int a){
        ...
        System.out.println("ENTRAMOS EN ADD APUESTA; turno vale "+this.turno+" por tanto se lo insertaremos a ese jugador la apuesta "+a);
        ...
        System.out.println("el valor de la apuesta del jug 0 es : "+this.apuestas[0]+"y ahora vamos a restarlo");
    ...
    public void siguienteTurno(){
        ...
        System.out.println("hacemos sig turno"+turno);
        boolean encontrado=false;
        if (this.getNumJugadores()==1) {
            this.turno=0;
            setFase(0) ;
                    ...
                    if (turno!=6){
                        ...
                        if (this.esSplit()){
                           ...
                           // if (this.manos[turno/2]!=null) encontrado=true;
                            if (((this.jugadoresRonda[posj]!=null )&& (this.apuestas[posj]>0))||(this.apuestas[posj]<=0 && this.jugadoresRonda[posj]!=null && !this.jugadoresRonda[posj].isEsperando() && this.jugadoresRonda[posj].isJugando()) ||(this.jugadoresRonda[posj]!=null && this.getFase()==0 && this.jugadoresRonda[posj].isJugando())){
                                encontrado=true;
                            }
                        }
   ...
   // return this.turno;
    ...
    public int plantarse(){
        ...
        if (!this.esSplit()){
            ...
            this.plantarse[turno/2]=true;
        }
        else{

            this.splitPlantarse[turno/2]=true;
    ...
    public void comprobarPagar(){
       ...
       while (i<3){
           ...
           if (this.splitManos[i]!=null && this.apuestasSplit[i]>0){
    ...
    public void pagar(int i){
        ...
        if ((valj>valb && !this.pasarse[i]) || (!this.pasarse[i]&& this.pasarse[3])){
        ...
        else if (valj==valb && !this.pasarse[i]){
    ...
    public void splitPagar(int i){
       ...
       if ((valj>valb && !this.splitPasarse[i]) || (!this.splitPasarse[i]&& this.pasarse[3])){
       ...
       else if (valj==valb && !this.splitPasarse[i]){
 ...
 */
    public Tcom generarCom(){
     ...
     int ultim=this.ultimoJugador();

     Tcom t = new Tcom(nicks,creditos,manosCom,apuestasCom, plantado, pasado, blackJack, doble, splitManosCom, hechoSplits, splitApuestas, splitDoble, splitPlantarseCom,splitPasarseCom, splitBlackJack, turnoCom,numJugadoresCom,  faseCom,  rondaEsperaCom,  idm, fechaPartida,ultim);
